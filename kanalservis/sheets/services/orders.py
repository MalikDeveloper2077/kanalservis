...
from ..utils.currency import get_rub_usd_rate
from ..utils.sheets import get_google_service_sacc, get_google_sheet_orders
...
def db_order_to_sheet_row(db_order: Order) -> list:
    return [
...
def sheet_order_row_to_dict(
    sheet_order_row: list,
    usd_rate: Union[int, float] = get_rub_usd_rate()
) -> dict:
    return {
        ...
        'delivery_date': datetime.strptime(sheet_order_row[DELIVERY_DATE],
...
class SheetOrderManager:
    ...
    def __init__(self, spreadsheet_id: str):
        self.sheet_orders = get_google_sheet_orders(google_service, spreadsheet_id)
...
class SheetOrderCreateManager(SheetOrderManager):
    def get_new_sheet_orders(self):
        return list(filter(
            lambda order: int(order[ORDER_NUMBER]) not in
            list(Order.objects.values_list('order_number', flat=True)), self.sheet_orders
        ))
...
class SheetOrderDeleteManager(SheetOrderManager):
    def get_deleted_sheet_orders_numbers(self):
        return list(filter(
            ...
            list(Order.objects.values_list('order_number', flat=True))
        ))
...
class SheetOrderUpdateManager(SheetOrderManager):
    def get_updated_sheet_orders_numbers(self) -> List[Tuple]:
        """Сопоставление всех данных заказа в таблице и в БД
        Возвращает кортежи в формате (order_number: int, sheet_order: dict)
        """
    ...
    def update_db(self):
        ...
        for order_number, sheet_order in updated_orders_numbers:
            Order.objects.filter(order_number=order_number).update(
                **sheet_order_row_to_dict(sheet_order)
            )
